...
#ifdef HAVE_PMEM
  bool is_pmem() const noexcept { return is_mmap() && !is_opened(); }
#else
...
# endif
  enum durability {
# ifndef __linux__ /* msync(MS_ASYNC) is no-op since Linux 2.6.19 */
    /** asynchronous write */
    WRITE,
# endif
    /** invoke a synchronous write after latch.wr_unlock() */
    DURABLE,
    /** synchronous write while holding exclusive latch */
    SYNC
  };
  /** Write or persist memory-mapped log.
  @param durable  whether this must be durable
  @return pending write LSN */
  lsn_t write_mmap(durability durable) noexcept;
#endif
...
public:
  ...
  void append(byte *&d, const void *s, size_t size) noexcept
  {
    ...
    ut_ad(d + size <= buf + (is_mmap() ? file_size : buf_size));
    memcpy(d, s, size);
    d+= size;
  }
