...
*/
static
bool innobase_check_foreigns_nullability(const dict_table_t *user_table,
                                         dict_foreign_t     **drop_fk,
                                         ulint              n_drop_fk,
                                         const char*        col_name,
                                         uint32_t           new_field_flags)
{
  ut_ad(mutex_own(&dict_sys.mutex));
  ...
  if ((new_field_flags & NOT_NULL_FLAG))
  {
    for (dict_foreign_t *foreign : user_table->foreign_set)
    {
      ...
      if ((foreign->type & DICT_FOREIGN_ON_UPDATE_CASCADE) &&
          foreign->check_fk_null_conflicts(col_name, true))
        goto non_null_error;
...
non_null_error:
  ...
  else
  {
    ...
    {
      if ((foreign->type & DICT_FOREIGN_ON_UPDATE_CASCADE)
          && foreign->check_fk_null_conflicts(col_name, false))
      {
...
*/
static
bool innobase_check_foreign_drop_col(const dict_table_t *user_table,
                                     dict_foreign_t     **drop_fk,
                                     ulint              n_drop_fk,
                                     const char*        col_name)
{
	...
	ulint			n_drop_fk)
{
	for (Field** fp = old_table->field; *fp; fp++) {
		...
		if (it == end) {
			if (innobase_check_foreign_drop_col(
			...
			if (innobase_check_foreigns_nullability(
				    user_table, drop_fk, n_drop_fk,
				    (*fp)->field_name.str, it->flags)) {
				return true;
			}
		}
	}
