...
# ifdef _WIN32
  void *ptr= MAP_FAILED;
  if (HANDLE h= CreateFileMappingA(file, nullptr, read_only
                                   ? PAGE_READONLY : PAGE_READWRITE,
                                   DWORD(size >> 32), DWORD(size), nullptr))
  {
    ptr= MapViewOfFileEx(h, read_only ? FILE_MAP_READ : FILE_MAP_WRITE,
                         0, 0, size, nullptr);
    CloseHandle(h);
    if (!ptr)
      ptr= MAP_FAILED;
...
void log_t::create(lsn_t lsn) noexcept
{
  ...
  if (is_mmap())
  {
    IF_WIN(,mprotect(buf, size_t(file_size), PROT_READ | PROT_WRITE));
...
# endif
    {
# ifdef _WIN32
      FlushViewOfFile(buf, 512);
      log.flush();
# else
      msync(buf, my_system_page_size, MS_SYNC);
# endif
    }
  }
  else
...
#ifdef HAVE_INNODB_MMAP
lsn_t log_t::write_mmap(durability durable) noexcept
{
  ut_ad(write_lock.is_owner());
  ut_ad(!durable || flush_lock.is_owner());
  ut_ad(latch_have_wr());

  const lsn_t end_lsn{get_lsn(std::memory_order_relaxed)};
  if (UNIV_LIKELY(durable != SYNC))
    latch.wr_unlock();
  write_lock.set_pending(end_lsn);
  const size_t ps_1{my_system_page_size - 1};
  const size_t start{size_t(calc_lsn_offset(write_lsn)) & ~ps_1};
  const size_t end{(size_t(calc_lsn_offset(end_lsn)) + ps_1) & ~ps_1};

# ifndef _WIN32
  const int flags=
#  ifndef __linux__
    durable == WRITE ? MS_ASYNC :
#  endif
    MS_SYNC;
  if (UNIV_UNLIKELY(end < start))
  {
    msync(buf + start, file_size - start, flags);
    msync(buf + START_OFFSET, end - START_OFFSET, flags);
  }
  else if (UNIV_LIKELY(end != start))
    msync(buf + start, end - start, flags);
# else
  /*
    We can do that flush range dance,
    or we can flush whole file does not affect performance
  */

  if (UNIV_UNLIKELY(end < start))
  {
    FlushViewOfFile(buf + start, file_size - start);
    FlushViewOfFile(buf + START_OFFSET, end - START_OFFSET);
  }
  else if (UNIV_LIKELY(end != start))
    FlushViewOfFile(log_sys.buf + start, end - start);

  if (durable)
    os_file_flush_func(log.m_file);
# endif

  write_lsn= end_lsn;

  lsn_t pending_write_lsn{write_lock.release(end_lsn)};
  lsn_t pending_flush_lsn= 0;
# ifndef __linux__
  switch (durable) {
  case WRITE:
    break;
  ...
  case SYNC:
# endif
    flushed_to_disk_lsn.store(end_lsn, std::memory_order_release);
    log_flush_notify(end_lsn);
    pending_flush_lsn = flush_lock.release(end_lsn);
# ifndef __linux__
  }
# endif
  return pending_write_lsn ? pending_write_lsn:pending_flush_lsn;
}
#endif
                     ...
                     const completion_callback *callback)
{
  ...
  if (!log_sys.is_mmap());
    ...
    return;
# ifdef HAVE_PMEM
  else if (!log_sys.is_opened())
  {
    log_sys.persist(lsn);
    return;
  }
# endif
#elif defined HAVE_PMEM
  if (log_sys.is_pmem())
  {
    if (durable)
      log_sys.persist(lsn);
...
# ifdef __linux__
      ut_ad(durable);
      pending_write_lsn= log_sys.write_mmap(log_sys.DURABLE);
# else
      pending_write_lsn=
        log_sys.write_mmap(durable ? log_sys.DURABLE : log_sys.WRITE);
# endif
      if (!pending_write_lsn)
        return;
      goto repeat_loop;
    }
    else
...
#ifdef HAVE_INNODB_MMAP
/** Try to enable or disable memory-mapped access (update log_mmap) */
ATTRIBUTE_COLD void log_t::set_mmap(bool mmap)
{
  if (is_pmem() || high_level_read_only)
    return;
  log_resize_acquire();
  if (!resize_in_progress() && is_opened() && bool(log_mmap) != mmap)
  {
    ...
    lsn_t end_lsn;
    {
      ...
      if (is_mmap())
      {
        IF_WIN(FlushViewOfFile(b, file_size), msync(b, file_size, MS_ASYNC));
        write_lsn= end_lsn= get_lsn(std::memory_order_relaxed);
      }
      else
      {
        ut_ad(bf < max_buf_free);
        end_lsn= write_buf<false>();
      }
    ...
    ut_ad(is_mmap() == bool(log_mmap));

    if (log_mmap)
    {
      const size_t new_buf_free{calc_lsn_offset(end_lsn)};
      set_buf_free(new_buf_free);
      ut_ad(!memcmp_aligned<16>(log_sys.buf + (new_buf_free & ~(bs - 1)),
                                log_block, new_buf_free & (bs - 1) & ~15));
      IF_WIN(,mprotect(buf, size_t(file_size), PROT_READ | PROT_WRITE));
    }
    else
    {
...
ATTRIBUTE_COLD void log_write_and_flush()
{
  ...
  if (!log_sys.is_mmap())
  {
    ...
    log_sys.write_mmap(log_sys.SYNC);
#endif
}
