                                 ...
                                 myf my_flags)
{
  ...
  array->mem_root= current_mem_root ? current_mem_root : _current_thd()->mem_root;
  ...
  */
  if (init_alloc &&
      !(array->buffer= (uchar*) alloc_root(current_mem_root? current_mem_root: _current_thd()->mem_root, init_alloc)))
    array->max_element=0;
...
int mem_root_allocate_dynamic(MEM_ROOT *mem_root, MEM_ROOT_DYNAMIC_ARRAY *array, size_t max_elements)
{
  ...
  if (max_elements >= array->max_element)
  {
    ...
    if (array->malloc_flags & MY_INIT_BUFFER_USED)
    {
       ...
       */
       if (!(new_ptr= (uchar *) alloc_root(mem_root ? mem_root : current_thd->mem_root,
...
{
  MEM_ROOT *mem_root= array->mem_root ? array->mem_root : _current_thd()->mem_root;
  if (idx >= array->max_element)
  {
    if (mem_root_allocate_dynamic(mem_root, array, idx))
      return TRUE;
    array->elements++;
  }
...
void* mem_root_dynamic_array_get_val(MEM_ROOT_DYNAMIC_ARRAY *array, size_t idx)
{
  ...
  void* element_ptr = array->buffer + (idx * array->size_of_element);
  return element_ptr;
}
