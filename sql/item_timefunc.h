...
public:
  Item_timestamp_with_fixed_tz_func(THD *thd, Item *a,
                                    const Type_time_zone_attributes &tzattr)
   :Item_func(thd, a), Type_time_zone_attributes(tzattr)
  { }
  const Type_handler *type_handler() const override
  {
  ...
  bool eq(const Item *item, bool binary_cmp) const override
  {
    return Item_func::eq(item, binary_cmp) &&
             Type_time_zone_attributes::operator==(
               static_cast<const Item_timestamp_with_fixed_tz_func&>(*item));
  }
  longlong val_int() override
  {
    return TH::item_value_null(current_thd, this, m_time_zone).to_longlong();
  }
  double val_real() override
  {
    return TH::item_value_null(current_thd, this, m_time_zone).to_double();
  }
  my_decimal *val_decimal(my_decimal *to) override
  {
    return TH::item_value_null(current_thd, this, m_time_zone).to_decimal(to);
  }
  String *val_str(String *to) override
  {
    return TH::item_value_null(current_thd, this, m_time_zone).
             val_str(to, (decimal_digit_t) decimals);
  }
  bool get_date(THD *thd, MYSQL_TIME *ltime, date_mode_t fuzzydate) override
  {
    return TH::item_value_null(current_thd, this, m_time_zone).get_date(ltime);
  }
};
...
public:
  ...
  LEX_CSTRING func_name_cstring() const override
  {
    static LEX_CSTRING name= {STRING_WITH_LEN("AT TIME ZONE")};
    return name;
  }
  bool check_arguments() const override;
  void print(String *str, enum_query_type query_type) override;
...
public:
  ...
  LEX_CSTRING func_name_cstring() const override
  {
    static LEX_CSTRING name= {STRING_WITH_LEN("timestamp_tz")};
    return name;
  }
  bool check_arguments() const override;
  void print(String *str, enum_query_type query_type) override;
