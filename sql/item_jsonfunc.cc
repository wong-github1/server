  ...
  while (a <= a_end)
  {
    ...
    if (a->type & JSON_PATH_ARRAY)
    {
      if (b->type & JSON_PATH_ARRAY)
      {
        ...
        if (array_sizes)
        {
          value_ptr= (int*)mem_root_dynamic_array_get_val(array_sizes,
                                                          b-temp_b);
...
longlong Item_func_json_equals::val_int()
{
  ...
  if (json_normalize(&a_res, a->ptr(), a->length(),
                     a->charset(), _current_thd()->active_stmt_arena_to_use()->mem_root))
  {
    null_value= 1;
    goto end;
  }
  ...
  if (json_normalize(&b_res, b->ptr(), b->length(),
                     b->charset(), _current_thd()->active_stmt_arena_to_use()->mem_root))
  {
    null_value= 1;
    goto end;
  }
...
String *Item_func_json_value::val_str(String *to)
{
  ...
  mem_root_dynamic_array_reset(&json_depth_array);

  null_value= Json_path_extractor::extract(&current_mem_root, to, args[0],
                                       args[1],
                                       collation.collation,
                           &json_depth_array,
                            &func_name_str);
                                  ...
                                  String *func_name_str)
{
  ...
  if (!parsed)
  {
    ...
    mem_root_dynamic_array_reset(&p.steps);
    if (s_p &&
                                          ...
                                          char **out_val, int *value_len)
{
  ...
  *type= possible_multiple_values ? JSON_VALUE_ARRAY : JSON_VALUE_NULL;
  ...
  while (json_get_path_next(&je, &p) == 0)
  {
    ...
    int zero_value= 0;
    int* value_ptr= (int*)mem_root_dynamic_array_get_val(&json_depth_array,
                                                         p.last_step - psteps);
    if (check_stack_overrun(current_thd, STACK_MIN_SIZE , NULL))
    {
      null_value= 1;
    ...
    if (has_negative_path && je.value_type == JSON_VALUE_ARRAY &&
        ...
        mem_root_dynamic_array_set_val(&json_depth_array,
                                       value_ptr ? value_ptr : &zero_value,
                                       p.last_step-psteps)))
      goto error;
...
static int check_contains(json_engine_t *js,
                          json_engine_t *value, json_engine_t *loc_js)
{
  bool set_js;
    ...
    *loc_js= *js;
    ...
    while (json_scan_next(value) == 0 && value->state != JST_OBJ_END)
    {
      ...
      if (!find_key_in_object(js, &key_name) ||
          json_read_value(js) ||
          !check_contains(js, value, loc_js))
        return FALSE;
    }
    ...
    if (value->value_type != JSON_VALUE_ARRAY)
    {
      *loc_js= *value;
      ...
      while (json_scan_next(js) == 0 && js->state != JST_ARRAY_END)
      {
        ...
        if (check_contains(js, value, loc_js))
        {
...
longlong Item_func_json_contains::val_int()
{
  ...
  result= check_contains(&je, &ve, &loc_js);
...
bool Item_func_json_contains_path::fix_length_and_dec(THD *thd)
{
  ...
  mem_root_dynamic_array_init(thd->active_stmt_arena_to_use()->mem_root, PSI_NOT_INSTRUMENTED,
                                &json_depth_array,
                                 sizeof(int), NULL,
                                 32, 32, MYF(0));
  mem_root_dynamic_array_init(thd->active_stmt_arena_to_use()->mem_root, PSI_NOT_INSTRUMENTED,
...
longlong Item_func_json_contains_path::val_int()
{
  ...
  while (json_get_path_next(&je, &p) == 0)
  {
    ...
    value_ptr= (int*)mem_root_dynamic_array_get_val(&json_depth_array,
                                                    p.last_step - psteps);
    if (has_negative_path && je.value_type == JSON_VALUE_ARRAY &&
        (json_skip_array_and_count(&je,
...
String *Item_func_json_array_append::val_str(String *str)
{
  ...
  {
    json_path_with_flags *c_path= paths + n_path;
    mem_root_dynamic_array_reset(&c_path->p.steps);
    if (!c_path->parsed)
    {
...
String *Item_func_json_search::val_str(String *str)
{
  ...
  while (json_get_path_next(&je, &p) == 0)
  {
    int value= 0;
    if (has_negative_path && je.value_type == JSON_VALUE_ARRAY &&
        (json_skip_array_and_count(&je, &value) ||
        mem_root_dynamic_array_set_val(&json_depth_array, &value,
                                        (p.last_step - (json_path_step_t*)p.steps.buffer))))
      goto js_error;

    if (json_value_scalar(&je))
    {
      if ((arg_count < 5 ||
           path_ok(paths, arg_count - 4, &p, je.value_type, &json_depth_array)) &&
          compare_json_value_wild(&je, s_str) != 0)
      {
        ++n_path_found;
        if (n_path_found == 1)
        {
           if (append_json_path(&temp, &p))
             goto js_error;
        }
        else
        {
          has_multi_value= true;
          if (temp.append(", ", 2) || append_json_path(&temp, &p))
            goto js_error;
        }
        if (mode_one)
          goto end;
...
String *Item_func_json_normalize::val_str(String *buf)
{
  ...
  if (json_normalize(&normalized_json,
                     ...
                     raw_json->charset(), _current_thd()->active_stmt_arena_to_use()->mem_root))
  {
    null_value= 1;
    goto end;
  }
...
longlong Item_func_json_overlaps::val_int()
{
  ...
  result= check_overlaps(&je, &ve, false, _current_thd()->active_stmt_arena_to_use()->mem_root);
...
longlong Item_func_json_schema_valid::val_int()
{
  ...
  if (!keyword_list.is_empty())
  {
    ...
    {
      if (curr_keyword->validate(&ve, _current_thd()->active_stmt_arena_to_use()->mem_root, NULL, NULL))
      {
        is_valid= 0;
        break;
      }
...
bool Item_func_json_schema_valid::fix_length_and_dec(THD *thd)
{
  ...
  json_scan_start(&je, js->charset(), (const uchar *) js->ptr(),
                  (const uchar *) js->ptr() + js->length());
  if (!create_object_and_handle_keyword(thd, thd->active_stmt_arena_to_use()->mem_root, &je, &keyword_list,
                                          &all_keywords))
    schema_parsed= true;
  else
    schema_parsed= false;
...
String* Item_func_json_key_value::val_str(String *str)
{
  ...
  null_value= Json_path_extractor::extract(&current_mem_root, &tmp_str,
                                           args[0], args[1],
                                        collation.collation,
                            &json_depth_array,
                             &func_name_str);
...
bool Item_func_json_key_value::fix_length_and_dec(THD *thd)
{
  ...
  String *js= args[0]->val_json(&tmp_val);
...
String* Item_func_json_array_intersect::val_str(String *str)
{
  ...
  if (get_intersect_between_arrays(str, &je2, items, _current_thd()->active_stmt_arena_to_use()->mem_root))
    goto error_return;
...
bool Item_func_json_object_filter_keys::fix_length_and_dec(THD *thd)
{
  ...
  if (json_read_value(&temp_je) || temp_je.value_type != JSON_VALUE_ARRAY ||
      create_hash(&temp_je, &items, hash_inited, &hash_root, thd->active_stmt_arena_to_use()->mem_root))
  {
    if (temp_je.s.error)
