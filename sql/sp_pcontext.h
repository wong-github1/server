...
public:
  /// Name of the record.
  Lex_ident_column name;
  Rec_definition_list *field;

public:
  sp_record(const Lex_ident_column &name_arg, Rec_definition_list *prmfield) 
   :Sql_alloc(),
    name(name_arg),
    field(prmfield)
  { }
...
public:
  ...
  */
  ...
  bool add_record(THD *thd, const Lex_ident_column &name, Rec_definition_list *field);

  sp_record *find_record(const LEX_CSTRING *name,
                                     bool current_scope_only) const;

  bool declare_record(THD *thd, const Lex_ident_column &name, Rec_definition_list *field)
  {
    if (find_record(&name, true))
    {
      // my_error(ER_SP_DUP_COND, MYF(0), name.str);  // kokseng todo
      return true;
    }
