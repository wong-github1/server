...
fk_check_column_changes(THD *thd, TABLE *table, Alter_info *alter_info,
                        FOREIGN_KEY_INFO *fk,
  ...
  while ((column= column_it++))
  {
	...
	limitation in MyISAM/ARIA. For the purposes of FK determination
      ...
      */
      ...
      else if (old_field_not_null != new_field_not_null)
      {
        if (referenced && !new_field_not_null)
        {
          ...
          */
          if (fk->update_method == FK_OPTION_CASCADE &&
              fk->referenced_fields_nullable &&
              !fk->is_nullable(fk->referenced_fields_nullable, n_col))
          {
            result= FK_COLUMN_DATA_CHANGE;
            goto func_exit;
          }
	  ...
	  */
          if (fk->update_method == FK_OPTION_SET_NULL ||
              ...
              (fk->update_method == FK_OPTION_CASCADE &&
               fk->foreign_fields_nullable &&
               fk->is_nullable(fk->foreign_fields_nullable, n_col)))
          {
