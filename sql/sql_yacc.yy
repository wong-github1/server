...
%type <NONE> set_assign_assoc_key
...
sp_hcond:
        ...
        | ident '.' ident /* CONDITION name */
          {
            $$= Lex->find_package_public_condition(thd, $1, &$3);
            if (unlikely($$ == NULL))
              MYSQL_YYABORT;
          }
...
sp_proc_stmt_open:
        ...
        | OPEN_SYM ident '.' ident opt_parenthesized_cursor_actual_parameters
          {
            if (unlikely(Lex->sp_open_cursor(thd, &$2, &$4, $5)))
              MYSQL_YYABORT;
          }
        ;
...
sp_proc_stmt_fetch_head:
        ...
        | FETCH_SYM ident '.' ident INTO
          {
            if (unlikely(Lex->sp_add_cfetch(thd, &$2, &$4)))
              MYSQL_YYABORT;
          }
        | FETCH_SYM FROM ident '.' ident INTO
          {
            if (unlikely(Lex->sp_add_cfetch(thd, &$3, &$5)))
              MYSQL_YYABORT;
          }
        | FETCH_SYM NEXT_SYM FROM ident '.' ident INTO
          {
            if (unlikely(Lex->sp_add_cfetch(thd, &$4, &$6)))
              MYSQL_YYABORT;
          }
        ;
...
sp_proc_stmt_close:
        ...
        | CLOSE_SYM ident '.' ident
          {
            if (Lex->sp_close_cursor(thd, &$2, &$4))
              MYSQL_YYABORT;
          }
        ;
...
explicit_cursor_attr:
        ...
        | ident_cli '.' ident PERCENT_ORACLE_SYM plsql_cursor_attr
          {
            if (unlikely(!($$= Lex->make_item_plsql_cursor_attr(thd, $1,
                                                                &$3, $5))))
              MYSQL_YYABORT;
          }
        ;
...
function_call_generic:
          ...
          opt_udf_expr_list ')' opt_result_field
          {
            ...
            */
            ...
            {
              if ($6)
                item= Lex->create_item_spvar_assoc_array_element_field(thd, &$1, $4, $6);
              else
                item= Lex->create_item_spvar_assoc_array_element(thd, &$1, $4);
            ...
            if ($6 && !allow_field_accessor)
            {
              my_error(ER_BAD_FIELD_ERROR, MYF(0), $6->str, $1);
              MYSQL_YYABORT;
            }
        ...
        ')'
          {
                ...
                spv->field_def.is_assoc_array())
            {
              ...
              !Lex->sp_package_public_type(thd, $1, $3, &field_type))
            {
              // Constructors for package's public TYPEs
              if (field_type.type_handler() == &type_handler_row)
              {
                if ($5 == NULL)
                {
                  my_error(ER_WRONG_ARGUMENTS, MYF(0), $3.str);
                  MYSQL_YYABORT;
                }
                else
                  $$= new (thd->mem_root) Item_row(thd, *$5);
              }
              else if (field_type.type_handler() == &type_handler_assoc_array)
              {
                if (unlikely($5 && Lex->sp_check_assoc_array_args($3, *$5)))
                  MYSQL_YYABORT;

                if ($5 == NULL)
                  $$= new (thd->mem_root) Item_assoc_array(thd);
                else
                  $$= new (thd->mem_root) Item_assoc_array(thd, *$5);
              }
              else
              {
                Lex->parse_error();
              ...
              (spv= Lex->find_package_public_variable($1, $3, &spec)) &&
              spv->field_def.is_assoc_array())
            {
            ...
            {
              if (unlikely(!($$= Lex->make_item_func_call_generic(thd,
                                                                  &$1,
                                                                  &$3,
                                                                  $5))))
                MYSQL_YYABORT;
        ...
        | ident_cli '.' ident_cli '.' ident_cli '(' opt_expr_list ')'
          {
            ...
            if ((thd->variables.sql_mode & MODE_ORACLE) &&
                ...
                spv->field_def.is_assoc_array())
            {
              if (unlikely(!($$= Lex->sp_get_assoc_array_method(thd, &$1,
                                                                &$3, &$5,
                                                                $7))))
                MYSQL_YYABORT;
            }
...
direct_call_statement:
          direct_call_or_assoc_init opt_sp_cparam_list_direct
          {
            if (!Lex->assoc_ident)
            {
              if (Lex->check_cte_dependencies_and_resolve_references())
...
set_assign_assoc_key:
          direct_call_or_assoc_init '(' opt_sp_cparams ')' SET_VAR
          {
            if (unlikely(Lex->assoc_ident == NULL))
            {
              thd->parse_error();
              MYSQL_YYABORT;
            }
            ...
            LEX *lex= Lex;
            lex->set_stmt_init();
            if (sp_create_assignment_lex(thd, Lex->assoc_ident->pos()))
              MYSQL_YYABORT;
            if (lex->sp_set_assoc_array_copy_key(Lex))
              MYSQL_YYABORT;
          }
          set_expr_or_default
          {
            if (unlikely(Lex->sp_set_assoc_array(thd,
                                                 Lex->assoc_ident,
                                                 $7.expr,
                                                 $7.expr_str)) ||
                unlikely(sp_create_assignment_instr(thd, yychar == YYEMPTY,
        ...
        | direct_call_or_assoc_init '(' opt_sp_cparams ')' '.'
          ident SET_VAR
          {
            if (unlikely(Lex->assoc_ident == NULL))
            {
          ...
          set_expr_or_default
          {
            if (unlikely(Lex->sp_set_assoc_array_field(thd,
                                                         Lex->assoc_ident,
                                                         &$6,
                                                         $9.expr,
                                                         $9.expr_str)) ||
...
assoc_name:
          TEXT_STRING_sys ARROW_SYM
        | LONG_NUM ARROW_SYM
        | ULONGLONG_NUM ARROW_SYM
        | DECIMAL_NUM ARROW_SYM
        | NUM  ARROW_SYM
        ;
...
assoc_array_table_types:
          field_type_all_with_record
          {
            Lex->last_field->set_attributes(thd, $1, COLUMN_DEFINITION_ROUTINE_LOCAL);
            $$= (Spvar_definition *)Lex->last_field;
          }
        | sp_decl_ident '.' ident PERCENT_ORACLE_SYM TYPE_SYM
          {
            if (unlikely(Lex->sphead->
              spvar_def_fill_type_reference(thd,
                                            ($$= (Spvar_definition *)Lex->last_field), $1,
                                            $3)))
              MYSQL_YYABORT;
          }
        | sp_decl_ident '.' ident '.' ident PERCENT_ORACLE_SYM TYPE_SYM
        ...
        | ident PERCENT_ORACLE_SYM ROWTYPE_ORACLE_SYM
          {
            if (unlikely(Lex->sphead->
                spvar_def_fill_rowtype_reference(thd,
                                                ($$= (Spvar_definition *)Lex->last_field),
                                                $1)))
              MYSQL_YYABORT;
          }
...
sp_decl_non_handler_no_cond:
        ...
        | typed_ident IS RECORD_SYM rec_type_body
          {
            if (unlikely(Lex->spcont->
                          declare_record(thd, Lex_ident_column(Lex->name), $4)))
              MYSQL_YYABORT;
        ...
        | typed_ident IS TABLE_SYM OF_SYM assoc_array_table_types
          ...
          {
            if (unlikely(Lex->spcont->
                          declare_assoc_array(thd,
                                              Lex_ident_column(Lex->name),
                                              $7, $5)))
              MYSQL_YYABORT;
            $$.vars= $$.conds= $$.hndlrs= $$.curs= 0;
          }
