...
public:
  ...
  */
  ...
  void set_composite_data_field_definitions(CompositeType *list)
  {
    DBUG_ASSERT(list);
    // set_handler(&type_handler_row);  // kokseng todo
    // m_row_field_definitions= list;

#if 0
    if constexpr (std::is_same_v<CompositeType, Row_definition_list>)
    {
        set_handler(&type_handler_row);
        m_row_field_definitions = list;
    }
    else if constexpr (std::is_same_v<T, Rec_definition_list>)
    {
        set_handler(&type_handler_record);
        m_rec_field_definitions = list;
    }
    else
    {
