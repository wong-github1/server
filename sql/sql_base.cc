...
{
  if (table->triggers->process_triggers(table->in_use, TRG_EVENT_UPDATE,
  ...
  */
  if (table->vfield && fields->elements)
  {
    ...
    if (item_field)
    {
      ...
      return table->update_virtual_fields(table->file, VCOL_UPDATE_FOR_WRITE);
    }
  }
  return 0;
}
                                     ...
                                     enum trg_event_type event)
{
  ...
  if (!result && table->triggers)
    result= invoke_before_triggers(thd, table, &fields, &values, ignore_errors,
                                   event);
  return result;
}
