...
{
  int count= 0;
  if (my_hash_init(&this->enum_values,
                                   ...
                                   const uchar* k_end)
{
  ...
  double val= my_strntod(je->s.cs, (char*)( je->value),  je->value_len, &end, &err);
                                       ...
                                       const uchar* k_end)
{
  ...
  longlong val= my_strntoll(je->s.cs, (char*)je->value, je->value_len, 10, &end, &err);
                                   ...
                                   const uchar* k_end)
{
  ...
  */
  ...
  if (!k_start && !k_end)
  {
    ...
    if (je->value_type != JSON_VALUE_STRING)
      return false;
     str->val_str(&tmp)->set_or_copy_aligned((const char*)je->value,
                                        (size_t)je->value_len, je->s.cs);
  }
  else
  {
    /* 2) */
    str->val_str(&tmp)->set_or_copy_aligned((const char*)k_start,
                                              ...
                                              *all_keywords)
{
  ...
  uint repertoire= my_charset_repertoire(je->s.cs);
                                        ...
                                        const uchar* k_end)
{
  ...
  if (my_hash_init(&unique_items, curr_je.s.cs,
                                                 ...
                                                 *all_keywords)
{
  ...
  if (my_hash_init(&this->properties,
                   je->s.cs, 1024, 0, 0,
    ...
    switch(je->state)
    {
      case JST_KEY:
      {
        ...
        if (curr_pattern_to_property)
        {
          ...
          curr_pattern_to_property->re.unset_flag(PCRE_CASELESS);
...
{
  if (my_hash_init(&all_keywords_hash,
