...
public:
  ...
  const Type_handler *type_handler() const override { return &type_handler_bool; }
  const Type_handler *fixed_type_handler() const override { return &type_handler_bool; }
...
public:
  ...
  bool eval_not_null_tables(void *) override { not_null_tables_cache= 0; return false; }
};
...
public:
  ...
  CHARSET_INFO *compare_collation() const override { return cmp.compare_collation(); }
...
public:
  ...
  const char *func_name() const override { return "trigcond"; };
...
{
protected:
  Item *find_item() const override { return args[0]->val_bool() ? args[1] : args[2]; }
...
{
protected:
  Item *find_item() const override { return args[0]->is_null() ? args[2] : args[1]; }
...
public:
  ...
  bool set(uint pos, Item *item) override;
  uchar *get_value(Item *item) override;
  const Type_handler *type_handler() const override { return &type_handler_datetime2; }
};
...
public:
  ...
  bool set(uint pos, Item *item) override;
  uchar *get_value(Item *item) override;
  const Type_handler *type_handler() const override { return &type_handler_time2; }
};
...
class Item_func_case_searched: public Item_func_case
{
  ...
  Item **else_expr_addr() const override { return with_else() ? &args[arg_count - 1] : 0; }
public:
...
public:
  ...
  CHARSET_INFO *compare_collation() const override { return cmp_collation.collation; }
};
...
public:
  ...
  longlong val_int() override;
  const char *func_name() const override { return "column_check"; }
...
public:
  ...
  longlong val_int() override;
  const char *func_name() const override { return "column_exists"; }
  bool need_parentheses_in_default() override { return false; }
  Item *get_copy(THD *thd) override
