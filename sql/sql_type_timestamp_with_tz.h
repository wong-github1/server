...
public:
  explicit Type_time_zone_attributes(const Time_zone *tz)
   :m_time_zone(tz)
  { }
  explicit Type_time_zone_attributes(const Type_extra_attributes &attr);
  bool operator==(const Type_time_zone_attributes &rhs) const
  {
    return m_time_zone == rhs.m_time_zone;
  }
  const Time_zone *time_zone() const
  {
    return m_time_zone;
  }
  bool join(const Type_time_zone_attributes &rhs)
  {
    if (m_time_zone)
      return (rhs.m_time_zone && m_time_zone != rhs.m_time_zone);
