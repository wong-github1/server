...
static Sys_var_charptr Sys_samurai_version_string(
       "samurai_version_string", "The version name of this Samurai release.",
       READ_ONLY GLOBAL_VAR(server_samurai_version_string_ptr),
       CMD_LINE_HELP_ONLY,
       IN_SYSTEM_CHARSET, DEFAULT(SAMURAI_VERSION_STRING));
...
static bool update_slave_retries_file(sys_var *self, THD* thd, enum_var_type type)
{
  if (!opt_slave_retries_path)
    opt_slave_retries_path=  my_strdup(default_slave_retries_path, MYF(0));
  return false;
}
...
static Sys_var_charptr Sys_slave_retries_log_path(
       ...
       CMD_LINE(REQUIRED_ARG, OPT_SLAVE_RETRIES_LOG), IN_SYSTEM_CHARSET, DEFAULT(NULL),
