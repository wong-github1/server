...
static const char *partition_engine_name = "partition";
...
bool process_database(
	...
	if (entry.type == Entry::Type::CREATE ||
		...
		if (!copy_file(ds, opt_file.c_str(), opt_file.c_str(), 0, true)) {
...
bool process_recopy(
	...
	if (entry.partitioned || entry.new_partitioned) {
		...
		for (const auto &file : *files) {
			...
			if (!copy_file(ds, file.c_str(), dst_path, 0, true)) {
	...
	for (const char *ext : ext_it->second) {
		...
		if (file_exists(file_name.c_str()) &&
			!copy_file(ds, file_name.c_str(), dst_path, 0, true)) {
	...
	if (file_exists(frm_file.c_str()) &&
		!copy_file(ds, frm_file.c_str(), frm_dst_path, 0, true)) {
...
bool process_rename(
	...
	DBUG_ASSERT(entry.db != partition_engine_name);
	...
	if (file_exists(new_frm_file.c_str()) &&
		!copy_file(ds, new_frm_file.c_str(), new_frm_dst, 0, true)) {
